...
public class GeoIPService extends AbstractService {
    ...
    private void init(String filePath) {
        ...
        try {
            ...
            if (!file.exists()) {
                logger.error("[gate] ip2country error: file {} not exists", filePath);
            } else {
                databaseFileModifyTime = file.lastModified();
            }
        } catch (Exception e) {
            logger.error("[gate] load ip2country error", e);
    ...
    public void reloadCountryCode() {
        ...
        try {
            ...
            logger.error("[gate] ip2country error: reload file {}", filePath, e);
        } finally {
    ...
    public String getCountryCode(String ip) {
        ...
        if (reader != null) {
            try {
                ...
                logger.error("[gate] GeoIP DatabaseReader not recognized ip:{}", ip);
            }
        } else {
            logger.info("[gate] GeoIP DatabaseReader is null");
        }
        return countryCode;
    }
    ...
    protected void initService() throws ServiceInitException {
        String geoFile = GateServer.getInstance().getConfiguration().getString("geoip.db.file",
