...
public class Kcp {
    ...
    private ByteBuf allocByteBuf() {
        return PooledByteBufAllocator.DEFAULT.buffer((mtu + IKCP_OVERHEAD) * 3);
    }
    ...
    public int receive(ByteBuf buffer) {
        ...
        // merge fragment.
        int len = 0;
        for (Iterator<Segment> it = rcv_queue.iterator(); it.hasNext(); ) {
            Segment seg = it.next();
        ...
        if (len != peekSize) {
            throw new RuntimeException("received data exception!");
        }
        // move available data from rcv_buf -> rcv_queue
        for (Iterator<Segment> it = rcv_buf.iterator(); it.hasNext(); ) {
            Segment seg = it.next();
    ...
    public int send(ByteBuf buffer) {
        ...
        int count = (buffer.readableBytes() <= mss) ? 1 : ((buffer.readableBytes() + mss - 1) / mss);
        if (count > 255) {
            return -2;
        }
        if (count == 0) {
    ...
    private void parse_una(int una) {
        for (Iterator<Segment> it = snd_buf.iterator(); it.hasNext(); ) {
            Segment seg = it.next();
    ...
    public int input(ByteBuf data) {
        ...
        while (true) {
            ...
            data.readInt(); // sessionId
            int conv_ = data.readInt();
            if (this.conv != conv_) {
                return -1;
            }
            byte cmd = data.readByte();
            byte frg = data.readByte(); // frg是fragment的缩小，是一个Segment在一次Send的data中的倒序序号
            int wnd = data.readShort(); // wnd是window的缩写； 滑动窗口大小，用于流控（Flow Control）
            int ts = data.readInt(); // 即timestamp , 当前Segment发送时的时间戳
            int sn = data.readInt(); // 即Sequence Number,Segment的编号
            int una = data.readInt(); // una即unacknowledged,表示此编号前的所有包都已收到了
    ...
    private void flush() {
        ...
        try {
            ...
            for (Iterator<Segment> it = snd_queue.iterator(); it.hasNext(); ) {
                if (_itimediff(snd_nxt, snd_una + cwnd_temp) >= 0) {
                    break;
                }
                Segment newseg = it.next();
                newseg.sessionId = sessionId;
                newseg.conv = conv;
            ...
            for (Segment segment : snd_buf) {
                ...
                if (segment.xmit == 0) {
                    ...
                    needsend = true;
                    segment.xmit++;
                    segment.rto += (nodelay == 0 ? rx_rto : rx_rto / 2);
